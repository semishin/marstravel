...
abstract class Acl_Core {
  ...
  */
	...
	public function is_allowed($role = NULL, $resource = NULL, $privilege = NULL)
	{
        ...
        $rule = $this->_find_match_role($resource,$roles,$privilege);

        if (!Auth::instance()->logged_in()) {
            if (
                Request::current()->action() == 'login'
                && Request::current()->directory() == 'Admin'
                && Request::current()->controller() == 'Auth'
            ) {
                if ($privilege == 'reset_password_step_1') {
                    return false;
                }
                return $rule['allow'];
            }
        ...
        if (Auth::instance()->logged_in('admin') && $privilege != 'login') {
            return true;
        }
        else {
