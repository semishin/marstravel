...
class JQFile
{
	...
	public function __construct($options = null, $initialize = true, $error_messages = null) {
		$this->options = array(
			'script_url' => PUBLIC_ROOT . 'temp/',
			'upload_dir' => PUBLIC_ROOT . 'temp/',
			'upload_url' => '/temp/',
			'user_dirs' => false,
			'mkdir_mode' => 0755,
			'param_name' => 'files',
	...
	protected function get_upload_path($file_name = null, $version = null) {
		...
		if (empty($version)) {
			...
			$version_dir = arr::get($this->options['image_versions'][$version], 'upload_dir');
			if ($version_dir) {
	...
	protected function get_download_url($file_name, $version = null, $direct = false) {
		...
		if (empty($version)) {
			...
			$version_url = arr::get($this->options['image_versions'][$version], 'upload_url');
			if ($version_url) {
	...
	protected function imagick_destroy_image_object($file_path) {
		$image = arr::get($this->image_objects, $file_path);
	...
	protected function imagemagick_create_scaled_image($file_name, $version, $options) {
		list($file_path, $new_file_path) =
			$this->get_scaled_image_file_paths($file_name, $version);
		$resize = arr::get($options, 'max_width')
