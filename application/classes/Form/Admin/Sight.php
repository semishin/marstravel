...
class Form_Admin_Sight extends CM_Form_Abstract
{
    protected function init()
    {
        ...
        $excursion = ORM::factory('Excursion')->order_by('name', 'ASC')->find_all();
        $excursion_list = array();
        foreach ($excursion as $item) {
            $excursion_list[$item->id] = $item->name;
        }
        ...
        $this->set_field('excursion', new CM_Field_Manytomany($excursion_list, $this->get_model()), 1);
